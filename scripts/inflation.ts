...
// Helper function to get CPI for a specific year and month
export function getCpi(year: number, initialMonth?: number): number {
  ...
  if (!yearData || !(month in yearData)) {
    throw new Error(`CPI data not found for year ${year} and month ${month}.`);
  }

  return yearData[month] as number;
}

// Main function to calculate inflation-adjusted value
export function calculateInflation(
  initialFrom: Partial<InflationInput>,
  initialTo: Partial<InflationInput> = { year: lastFullYear }
): number {
